...
from assistant.core.keyboard import ikb
...
async def start(_, message):
    ...
    if len(message.text.split()) > 1:
        user = await asstb.get_users(message.from_user.id)
        name = (message.text.split(None, 1)[1]).lower()
        match = re.match(r"rules_(.*)", name)
        if match:
            chat_id = match.group(1)
            user_id = message.from_user.id
            chat = await asstb.get_chat(int(chat_id))
...
@asstb.on_callback_query(filters.regex("bot_commands"))
async def commands_callbacc(_, CallbackQuery):
    text, keyboard = await help_parser(CallbackQuery.from_user.mention)
    await asstb.send_message(
